import {
  aliasLimitWorkaround,
  emailMapAliasLimitWorkaround,
} from '../src/AliasLimitWorkaround';
import {
  EmailMap,
  EmailUser,
  EmailUserAlias,
...
const alias2: EmailUserAlias = {
  kind: 'EmailUserAlias',
  email: alias2Email,
  user: user1,
};
const user2: EmailUser = { kind: 'EmailUser', email: userEmail2 };
const alias3: EmailUserAlias = {
  kind: 'EmailUserAlias',
  email: alias3Email,
  user: user2,
};
const alias4: EmailUserAlias = {
  kind: 'EmailUserAlias',
  email: alias4Email,
  user: user2,
};
...
const aliasGroup1: EmailGroup = {
  ...
  email: aliasGroup1Email,
  members: [user1],
};
const alias2OnGroup1: EmailGroupAlias = {
...
const aliasGroup2: EmailGroup = {
  ...
  email: aliasGroup2Email,
  members: [user2],
};
const alias4OnGroup2: EmailGroupAlias = {
  kind: 'EmailGroupAlias',
  email: alias4Email,
  group: aliasGroup2,
};
...
describe('Workaround for limit on aliases per entity', () => {
  it('accepts empty data as an EmailMap', () => {
    expect(emailMapAliasLimitWorkaround({}, config)).toStrictEqual(
      {} as EmailMap,
    );
  });
  ...
  it('accepts simple data as an EmailMap', () => {
    ...
    expect(emailMapAliasLimitWorkaround(passThroughData, config)).toStrictEqual(
      passThroughData,
    );
  });
  ...
  it('Handles simple workaround', () => {
    expect(aliasLimitWorkaround([alias1, alias2], config)).toStrictEqual([
      alias1,
      aliasGroup1,
      alias2OnGroup1,
    ]);
  ...
  it('Handles 2 users', () => {
    expect(
      aliasLimitWorkaround([alias1, alias2, alias3, alias4], config),
    ).toStrictEqual([
      alias1,
      aliasGroup1,
      alias2OnGroup1,
