...
import { WorkmailEmailmap } from '../src/WorkmailMap';
...
describe('Synchronizing 2 AwsMaps', () => {
  ...
  it('creates alias', () => {
    ...
    let target: WorkmailEmailmap = {"foo@bar": {kind: "WorkmailUserAlias", email: "foo@bar", userEntityId: "eid"}}
  ...
  it('removes alias', () => {
    let current: WorkmailEmailmap = {"foo@bar": {kind: "WorkmailUserAlias", email: "foo@bar", userEntityId: "eid"}}
  ...
  it('removes and creates same alias for another user', () => {
    let current: WorkmailEmailmap = {"foo@bar": {kind: "WorkmailUserAlias", email: "foo@bar", userEntityId: "eid1"}}
    let target: WorkmailEmailmap = {"foo@bar": {kind: "WorkmailUserAlias", email: "foo@bar", userEntityId: "eid2"}}
